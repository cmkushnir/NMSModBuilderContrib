...
// Any mod script with a name enclosed in [] is only Executed if explicitly called
// from another script via Execute<name>().
//
...
public class Configure : NMS.Script.ModClass
{
	...
	protected static void OnAfterBuildPakStream( NMS.Script.ModFiles SCRIPTS, CancellationToken CANCEL )
	{
		...
		foreach( var entry in SCRIPTS.DataCache ) {
			...
			if( !data.IsEdited ||   // data.Save was called when building stream, so can just check IsEdited here
				mods.Count < 2      // skip if only 1 script extracted | created
			)	continue;
		...
		// save this same info to disk instead of just to log window
