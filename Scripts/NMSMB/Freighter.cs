...
public class Freighter : cmk.NMS.Script.ModClass
{
	...
	protected void GcRealityManagerData()
	{
		...
		foreach( var option in mbin.FreighterCargoOptions ) {
			option.MinAmount *= 2;
			option.MaxAmount *= 4;
		}
	}
	...
	protected void GcFleetGlobals()
	{
		...
		mbin.LevelupProgressRequiredToNotBeSadAboutDamage =  0.80f;  //  0.95
		
		//mbin.ExpeditionsCompleteInstantly = true;  // false
		mbin.TimeTakenForExpeditionEvent    = 60;    // 5400
		//for( var i = 0; i < mbin.ExpeditionDurations.Duration.Length; ++i ) {
		//	mbin.ExpeditionDurations.Duration[i] = 1;
		//}
