...
public class Inventory_Stack : cmk.NMS.Script.ModClass
{
	...
	protected void GcGameplayGlobals()
	{
		...
		mbin.TechMustBeInTechInventory = false;
		
		// starting in 4.0
		var high_stacks = mbin.DifficultyConfig.InventoryStackLimitsOptionData[(int)InventoryStackLimitsOptionDataEnum.High];
		high_stacks.SubstanceStackLimit = MaxAmount;
		high_stacks.ProductStackLimit   = MaxAmount;
		for( var i = 0; i < high_stacks.MaxSubstanceStackSizes.Length; ++i ) {
			high_stacks.MaxSubstanceStackSizes[i] = MaxAmount;
		}
		for( var i = 0; i < high_stacks.MaxProductStackSizes.Length; ++i ) {
			high_stacks.MaxProductStackSizes[i] = 1;
		}
	}
	...
	protected void GcInventoryStoreBalance()
	{
		...
		foreach( var path in paths ) {
			...
			mbin.PlayerPersonalInventoryTechWidth   = 10;  // 10
			mbin.PlayerPersonalInventoryTechHeight  = 10;  //  5
			mbin.PlayerPersonalInventoryCargoWidth  = 10;  //  8
			mbin.PlayerPersonalInventoryCargoHeight = 12;  //  6
			...
			// as of 4.0 these are moved to GCGAMEPLAYGLOBALS.GLOBAL.MBIN
	...
	protected void GcSubstanceTable( int STACK_MULTIPLIER )
	{
		...
		foreach( var substance in mbin.Table ) {
			...
			if( substance.StackMultiplier > 0 ) {
				substance.StackMultiplier = STACK_MULTIPLIER;
			}
		}
	}
