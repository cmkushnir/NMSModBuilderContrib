...
public class Add_Tech_Rockets : cmk.NMS.Script.ModClass
{
	...
	protected void GcRealityManagerData()
	{
		...
		if( AddShipRocketTech ) {
			mbin.AvailableTech.Add("UT_ROCKETS_DMG");
		}
		if( AddShipRocketProcTech ) {  // only get from warriors
			mbin.TradeSettings.LoneWar.OptionalProducts.Add("U_SROCKETSX");
			mbin.TradeSettings.WarShip.OptionalProducts.Add("U_SROCKETSX");
			mbin.TradeSettings.IllegalProds.AlwaysPresentProducts.Add("U_SROCKETSX");
			mbin.TradeSettings.PirateTech.AlwaysPresentProducts.Add("U_SROCKETSX");
		}
	}
